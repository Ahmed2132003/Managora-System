...
import type { DocumentCategory, EmployeeDocument, LinkedEntityType } from "../../../../shared/hr/hooks";

type DocumentsSectionProps = {
  content: PageContent;
  language: Language;
  employeeId?: number;
  documentForm: DocumentFormReturn;
  handleUploadDocument: (values: unknown) => void;
  uploadDocumentMutation: MutationLike;
  documentTypeOptionsByLanguage: Record<Language, Option<"contract" | "id" | "other">[]>;
  documentCategoryOptionsByLanguage: Record<Language, Option<DocumentCategory>[]>;
  linkedEntityOptionsByLanguage: Record<Language, Option<LinkedEntityType>[]>;
  documentCategoryFilter: string;
  setDocumentCategoryFilter: (value: string) => void;
  documentSearch: string;
  setDocumentSearch: (value: string) => void;
  documentsQuery: DocumentsQueryLike;
  handleDeleteDocument: (documentId: number) => void;
  deleteDocumentMutation: MutationLike;
};
    ...
    <section className="panel employee-profile__subpanel">
      ...
      <div className="employee-profile__documents">
        <div className="employee-profile__document-header">
          <label className="form-field"><span>{content.documents.docType}</span><Controller name="doc_type" control={documentForm.control} render={({ field }) => <select value={field.value} onChange={(event) => field.onChange(event.target.value)} disabled={!employeeId}>{documentTypeOptionsByLanguage[language].map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>} /></label>
          <label className="form-field"><span>{content.documents.category}</span><Controller name="category" control={documentForm.control} render={({ field }) => <select value={field.value} onChange={(event) => field.onChange(event.target.value)} disabled={!employeeId}>{documentCategoryOptionsByLanguage[language].map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>} /></label>
          <label className="form-field"><span>{content.documents.linkedType}</span><Controller name="linked_entity_type" control={documentForm.control} render={({ field }) => <select value={field.value ?? ""} onChange={(event) => field.onChange(event.target.value)} disabled={!employeeId}><option value="">-</option>{linkedEntityOptionsByLanguage[language].map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>} /></label>
        ...
        <div className="employee-profile__document-filters">
          <label className="form-field"><span>{content.documents.category}</span><select value={documentCategoryFilter} onChange={(event) => setDocumentCategoryFilter(event.target.value)}><option value="">{content.documents.allCategories}</option>{documentCategoryOptionsByLanguage[language].map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
