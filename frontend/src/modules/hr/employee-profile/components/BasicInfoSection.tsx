...
type BasicInfoSectionProps = {
  ...
  statusOptions: Option<EmployeeStatus>[];
...
export function BasicInfoSection({ content, form, statusOptions, userOptions, userSelectDisabled, departmentOptions, jobTitleOptions, shiftOptions }: BasicInfoSectionProps) {
  return (
    <section className="panel employee-profile__subpanel">
      ...
      <div className="employee-profile__grid">
        ...
        <label className="form-field"><span>{content.fields.status}</span><Controller name="status" control={form.control} render={({ field }) => <select value={field.value} onChange={(event) => field.onChange(event.target.value)}>{statusOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>} /></label>
        <label className="form-field"><span>{content.fields.manager}</span><Controller name="manager_id" control={form.control} render={({ field }) => <select value={field.value ?? ""} onChange={(event) => field.onChange(event.target.value || null)}><option value="">{content.fields.managerPlaceholder}</option>{userOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>} /></label>
        <label className="form-field"><span>{content.fields.user}</span><Controller name="user_id" control={form.control} render={({ field }) => <select value={field.value} onChange={(event) => field.onChange(event.target.value)} disabled={userSelectDisabled}><option value="">{content.fields.userPlaceholder}</option>{userOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>} /></label>
