...
export function PayrollSection(props: any) {
  ...
  return <>
    <section className="panel employee-profile__subpanel">
      ...
      <div className="employee-profile__grid">
        <label className="form-field"><span>{content.fields.salaryType}</span><Controller name="salary_type" control={salaryForm.control} render={({ field }) => <select value={field.value ?? ""} onChange={(event) => field.onChange(event.target.value)} disabled={!employeeId}><option value="" disabled>{content.payroll.salaryTypePlaceholder}</option>{salaryTypeOptions.map((option: any) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>} /></label>
    ...
    <section className="panel employee-profile__subpanel"><div className="panel__header"><div><h2>{content.adjustments.title}</h2><p>{content.adjustments.subtitle}</p></div></div>
      <div className="employee-profile__grid">
        <label className="form-field"><span>{content.adjustments.typeLabel}</span><select value={adjustmentType} onChange={(event) => setAdjustmentType(event.target.value)}><option value="bonus">{content.adjustments.bonusType}</option><option value="deduction">{content.adjustments.deductionType}</option><option value="advance">{content.adjustments.advanceType}</option></select></label>
        {adjustmentType !== "advance" ? <>
          ...
          <label className="form-field"><span>{content.adjustments.periodLabel}</span><select value={adjustmentPeriodId ?? ""} onChange={(event) => setAdjustmentPeriodId(event.target.value ? Number(event.target.value) : null)}><option value="">{content.adjustments.periodPlaceholder}</option>{availableAdjustmentPeriods.map((period: any) => <option key={period.id} value={period.id}>{period.period_type === "monthly" ? `${period.year}-${String(period.month).padStart(2, "0")}` : `${period.start_date} → ${period.end_date}`}</option>)}</select></label>
        </> : <>
