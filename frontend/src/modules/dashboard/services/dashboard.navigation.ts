import type { Content, NavLink } from "../types/dashboard.types";

export function getDashboardNavLinks(content: Content): NavLink[] {
  return [
    { path: "/dashboard", label: content.nav.dashboard, icon: "🏠" },
    { path: "/admin", label: content.nav.adminPanel, icon: "🛠️", superuserOnly: true },
    { path: "/users", label: content.nav.users, icon: "👥", permissions: ["users.view"] },
    { path: "/attendance/self", label: content.nav.attendanceSelf, icon: "🕒" },
    { path: "/employee/self-service", label: content.nav.employeeSelfService, icon: "🧑‍💼" },
