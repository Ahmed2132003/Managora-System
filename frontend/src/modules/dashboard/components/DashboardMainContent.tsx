  ...
  return (
    <main className="dashboard-main">
      ...
      <section className="panel panel--command-center panel--wide">
        ...
        <div className="command-center-grid">{data.commandCards.map((card) => <div key={card.label} className="command-center-card"><span>{card.label}</span><strong>{card.value}</strong></div>)}</div>
        <div className="executive-bars">{data.commandCards.map((card, index: number) => <div key={`cmd-${card.label}`} className="executive-bars__row"><span>{card.label}</span><div className="executive-bars__track"><span style={{ width: `${42 + (((index + 2) * 10) % 43)}%` }} /></div></div>)}</div>
      </section>
      ...
      <section className="grid-panels">
        <div className="panel panel--insights">
          ...
          <div className="bar-chart">{data.barValues.length ? data.barValues.map((item) => <span key={item.date} style={{ height: `${item.height}%` }} title={`${item.date}: ${formatCurrency(item.value.toString())}`} />) : <span className="bar-chart__empty">{data.kpisQuery.isLoading ? content.loadingLabel : content.searchEmptyTitle}</span>}</div>
        </div>
        ...
        <div className="panel panel--forecast panel--wide">
          ...
          <div className="forecast-grid">{data.forecastCards.length ? data.forecastCards.map((card) => <div key={card.label} className="forecast-card"><span>{card.label}</span><strong>{card.value}</strong></div>) : <div className="forecast-card"><span>{content.loadingLabel}</span><strong>-</strong></div>}</div>
          <div className="executive-bars">{data.forecastBars.map((bar) => <div key={bar.label} className="executive-bars__row"><span>{bar.label}</span><div className={`executive-bars__track executive-bars__track--${bar.tone}`}><span style={{ width: `${bar.width}%` }} /></div></div>)}</div>
        </div>
        ...
        <div className="panel panel--signals panel--wide">
          ...
          <div className="signal-bars">{data.riskDistribution.map((item) => <div key={item.label} className="signal-bar-item"><div className="signal-bar-item__meta"><span>{item.label}</span><strong>{item.value}</strong></div><div className="signal-bar-track"><span style={{ width: `${Math.max(item.ratio, item.value ? 8 : 0)}%` }} /></div></div>)}</div>
        </div>
        ...
        <div className="panel panel--activity panel--wide">
          ...
          <div className="activity-list">{data.activityItems.length ? data.activityItems.map((item) => <div key={item.id} className="activity-item"><div><strong>{item.title}</strong><span>{new Date(item.event_date).toLocaleDateString(isArabic ? "ar" : "en")}</span></div><span className="tag">{item.severity}</span></div>) : <div className="activity-item"><div><strong>{content.searchEmptyTitle}</strong><span>{content.searchEmptySubtitle}</span></div><span className="tag">-</span></div>}</div>
        </div>
      </section>
    </main>
  );
